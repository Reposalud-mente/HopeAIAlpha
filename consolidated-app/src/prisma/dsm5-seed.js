...
async function main() {
  ...
  // Get all existing ICD criteria codes
  const existingICDCodes = await prisma.iCDCriteria.findMany({
    select: { code: true }
  });

  const icdCodeSet = new Set(existingICDCodes.map(icd => icd.code));

  // Seed DSM-5 criteria
  let seededCount = 0;
  for (const criteria of dsm5Criteria) {
    // Check if the ICD equivalent exists
    if (criteria.icdEquivalent && !icdCodeSet.has(criteria.icdEquivalent)) {
      console.log(`Warning: ICD code ${criteria.icdEquivalent} not found for DSM-5 code ${criteria.code}. Setting icdEquivalent to null.`);
      // Create a copy of the criteria without the icdEquivalent
      const criteriaWithoutICD = { ...criteria, icdEquivalent: null };

      await prisma.dSM5Criteria.upsert({
        where: { code: criteria.code },
        update: criteriaWithoutICD,
        create: criteriaWithoutICD,
      });
    } else {
      // ICD code exists or is null, proceed normally
      await prisma.dSM5Criteria.upsert({
        where: { code: criteria.code },
        update: criteria,
        create: criteria,
  ...
  console.log(`Seeded ${seededCount} DSM-5 criteria`);
